...
SELECT t7.id,
    ...
    t7.status
   FROM dblink(
        'host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=electrep_new user=klpwww password=44K5z25bhPTGcyoG'::text, 'select id, parent, dise_slug, elec_comm_code, const_ward_name, const_ward_type, neighbours, current_elected_rep, current_elected_party, status from tb_electedrep_master'::text
    ) t7(
        id integer,
        parent integer,
        dise_slug text,
...
SELECT t8.sid,
    ...
    t8.heirarchy,
    t8.gram_panchayat
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=electrep_new user=klpwww password=44K5z25bhPTGcyoG'::text, 'select sid, ward_id, mla_const_id, mp_const_id, heirarchy, gram_panchayat from tb_school_electedrep'::text) t8(sid integer, ward_id integer, mla_const_id integer, mp_const_id integer, heirarchy integer, gram_panchayat text);
...
CREATE MATERIALIZED VIEW mvw_assembly AS
 SELECT mvw_electedrep_master.id, gid, ac_id,
    ...
    state_ut,
    the_geom
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=spatial user=klpwww password=44K5z25bhPTGcyoG '::text, 'select gid, ac_id, ac_no, ac_name, state_ut, the_geom from assembly'::text)
        assembly(
            gid integer,
            ac_id integer,
            ac_no integer,
...
CREATE MATERIALIZED VIEW mvw_parliament AS
 SELECT mvw_electedrep_master.id, gid, pc_id,
    ...
    state_ut,
    the_geom
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=spatial user=klpwww password=44K5z25bhPTGcyoG'::text, 'select gid, pc_id, pc_no, pc_name, state_ut, the_geom from parliament'::text)
        parliament(
            gid integer,
            pc_id integer,
            pc_no integer,
...
CREATE MATERIALIZED VIEW mvw_postal AS
 SELECT gid,
    ...
    pincode,
    the_geom
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=spatial user=klpwww password=44K5z25bhPTGcyoG'::text, 'select gid, pin_id, pincode, the_geom from postal'::text) postal(gid integer, pin_id integer, pincode character varying(35), the_geom geometry);

DROP MATERIALIZED VIEW IF EXISTS mvw_inst_coord CASCADE;
CREATE materialized VIEW mvw_inst_coord AS
SELECT t2.instid,
       t2.coord
FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=klp-coord user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from inst_coord'::text) t2(instid integer, coord geometry);
...
SELECT t1.dise_code,
    ...
    t1.status,
    t1.rte_group
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=dise_all user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_dise_rte_agg'::text) t1(dise_code character varying(32), rte_metric character varying(36), status character varying(30), rte_group character varying(32));
...
SELECT t1.dise_code,
    ...
    t1.score,
    t1.df_group
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=dise_all user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_dise_facility_agg'::text) t1(dise_code character varying(32), df_metric character varying(30), score numeric(5,0), df_group character varying(30));
...
FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=apmdm user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_mdm_agg'::text) t1(id integer, mon character varying(15), wk integer, indent integer, attend integer);
...
SELECT t1.school_code as dise_code,
    ...
    t1.books_in_library
FROM dblink(
    'host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=klpdise_olap user=klpwww password=44K5z25bhPTGcyoG'::text,
    'SELECT
        school_code,
        assembly_name,
        parliament_name,
...
SELECT t1.sid,
    ...
    t1.ai_group
FROM dblink(
   'host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=ang_infra user=klpwww password=44K5z25bhPTGcyoG'::text,
...
SELECT t1.key,
    t1.value
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=ang_infra user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_display_master'::text) t1(key character varying(32), value character varying(200));
...
SELECT t1.trans_year,
    ...
    t1.school_name,
    t1.klp_child_id
FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=library user=klpwww password=44K5z25bhPTGcyoG'::text, 'select trans_year,class,issue_date,klp_school_id,school_name,klp_child_id from libentry where flag is not null'::text) t1(trans_year character varying(30), class numeric(3,0), issue_date character varying(20), klp_school_id numeric(7,0), school_name character varying(50), klp_child_id character varying(30));
...
SELECT t1.klp_school_id,
    ...
    t1.book_lang,
    t1.child_count
FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=library user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from lang_agg'::text) t1(klp_school_id integer, class integer, month character varying(10), year character varying(10), book_lang character varying(50), child_count integer);
...
SELECT t1.klp_school_id,
    ...
    t1.book_level,
    t1.child_count
FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=library user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from level_agg'::text) t1(klp_school_id integer, class integer, month character varying(10), year character varying(10), book_level character varying(50), child_count integer);
...
CREATE MATERIALIZED VIEW mvw_boundary_coord AS SELECT t1.id_bndry,
    t1.type,
    t1.coord
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=klp-coord user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from boundary_coord'::text) t1(id_bndry integer, type character varying(20), coord geometry);
...
CREATE MATERIALIZED VIEW mvw_dise_display_master AS SELECT t1.key,
    t1.value
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=dise_all user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_display_master'::text) t1(key character varying(36), value character varying(200));
...
CREATE MATERIALIZED VIEW mvw_paisa_data AS SELECT t1.grant_type,
    ...
    t1.operator,
    t1.factor
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=dise_all user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_paisa_data'::text) t1(grant_type character varying(32), grant_amount integer, criteria character varying(32), operator character varying(3), factor character varying(32));
...
CREATE MATERIALIZED VIEW mvw_school_eval AS SELECT t.sid,
    ...
    t.qid,
    t.value
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=pratham_mysore user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_school_eval'::text) t(sid integer, disecode character varying(100), domain character varying(100), qid integer, value character varying(50));
...
CREATE MATERIALIZED VIEW mvw_anginfra_agg AS SELECT t1.sid,
    ...
    t1.perc_score,
    t1.ai_group
   FROM dblink('host=www-db.cs6dawfr0djr.ap-south-1.rds.amazonaws.com dbname=ang_infra user=klpwww password=44K5z25bhPTGcyoG'::text, 'select * from tb_ang_infra_agg'::text) t1(sid integer, ai_metric character varying(30), perc_score numeric(5,0), ai_group character varying(30));
